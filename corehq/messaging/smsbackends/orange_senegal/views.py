from corehq.apps.ivr.api import log_call
from corehq.apps.sms.api import incoming as incoming_sms
from corehq.apps.sms.views import IncomingBackendView
from corehq.messaging.smsbackends.orange_senegal.models import SQLOrangeSNBackend
from django.http import HttpResponse


EMPTY_RESPONSE = """<?xml version="1.0" encoding="UTF-8" ?>
<Response></Response>"""

IVR_RESPONSE = """<?xml version="1.0" encoding="UTF-8" ?>
<Response>
    <Pause length="30" />
    <Reject />
</Response>"""


class OrangeSNIncomingSMSView(IncomingBackendView):
    urlname = 'orange_sn_sms'

    @property
    def backend_class(self):
        return SQLOrangeSNBackend

    def post(self, request, api_key, *args, **kwargs):
        message_sid = request.POST.get('MessageSid')
        from_ = SQLOrangeSNBackend.convert_from_whatsapp(request.POST.get('From'))
        body = request.POST.get('Body')
        incoming_sms(
            from_,
            body,
            SQLOrangeSNBackend.get_api_id(),
            backend_message_id=message_sid,
            domain_scope=self.domain,
            backend_id=self.backend_couch_id
        )
        return HttpResponse(EMPTY_RESPONSE)


class OrangeSNIncomingIVRView(IncomingBackendView):
    urlname = 'orange_sn_ivr'

    @property
    def backend_class(self):
        return SQLOrangeSNBackend

    def post(self, request, api_key, *args, **kwargs):
        from_number = request.POST.get('From')
        call_sid = request.POST.get('CallSid')
        log_call(from_number, '%s-%s' % (SQLOrangeSNBackend.get_api_id(), call_sid))
        return HttpResponse(IVR_RESPONSE)
